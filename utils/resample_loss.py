...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_bias = - torch.log(self.train_num / (self.class_freq + 1e-7) - 1) * init_bias
        self.freq_inv = torch.ones(self.class_freq.shape) / (self.class_freq + 1e-7)
        self.propotion_inv = self.train_num / (self.class_freq + 1e-7)
    ...
    def forward(self,
        ...
        if self.focal:
            ...
            pt = pt / (torch.sum(pt, dim=1, keepdim=True) + 1e-7)
    ...
    def reweight_functions(self, label):
        ...
        if self.weight_norm is not None:
            if 'by_instance' in self.weight_norm:
                ...
                weight = weight / (max_by_instance + 1e-7)
            elif 'by_batch' in self.weight_norm:
                weight = weight / (torch.max(weight) + 1e-7)
        return weight
    ...
    def rebalance_weight(self, gt_labels):
        ...
        pos_weight = self.freq_inv.clone().detach().unsqueeze(0).to(gt_labels.device) / (repeat_rate + 1e-7)
    ...
    def CB_weight(self, gt_labels):
        if 'by_class' in self.CB_mode:
            weight = torch.tensor((1 - self.CB_beta)) / (1 - torch.pow(self.CB_beta, self.class_freq) + 1e-7)
        elif 'average_n' in self.CB_mode:
            avg_n = torch.sum(gt_labels * self.class_freq, dim=1, keepdim=True) / torch.sum(gt_labels + 1e-7, dim=1,
                                                                                            keepdim=True)
            weight = torch.tensor((1 - self.CB_beta)) / (1 - torch.pow(self.CB_beta, avg_n) + 1e-7)
        elif 'average_w' in self.CB_mode:
            weight_ = torch.tensor((1 - self.CB_beta)) / (1 - torch.pow(self.CB_beta, self.class_freq) + 1e-7)
            weight = torch.sum(gt_labels * weight_, dim=1, keepdim=True) / torch.sum(gt_labels + 1e-7, dim=1,
                                                                                     keepdim=True)
        elif 'min_n' in self.CB_mode:
            min_n, _ = torch.min(gt_labels * self.class_freq +
                                 (1 - gt_labels) * 100000, dim=1, keepdim=True)
            weight = torch.tensor((1 - self.CB_beta)) / \
                     (1 - torch.pow(self.CB_beta, min_n) + 1e-7)
        else:
            raise NameError
        return weight
    ...
    def RW_weight(self, gt_labels, by_class=True):
        ...
        if not by_class:
            ...
            weight = sum_ / torch.sum(gt_labels + 1e-7, dim=1, keepdim=True)
        return weight
