...
from methods.dice_vi import DICE_VI
...
def get_model():
    model = DICE_VI(loss_func=resample_loss(loss_type=loss_type,
...
if __name__ == '__main__':
    ...
    for split in range(len(x_trains)):
        train_dir = os.path.join('save', f'{algorithm}_{loss_type}', dataset, str(split),
                                 f'{lr}_{adjust_lr}_{batch_size}_{max_epoch}_{lambda1}_{lambda2}_{seed}',
                                 'train')
        ...
        for i in range(len(XTrain)):
            train_dataset = TensorDataset(torch.from_numpy(XTrain[i]).float(),
                                          torch.from_numpy(YTrain[i]))
        ...
        print(f'{dataset}, {split}, {algorithm}, {max_epoch}, {seed}')
